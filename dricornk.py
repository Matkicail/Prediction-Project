...
# DRICORN-K our baseline to modify on
# TODO make this a DRICORN-K 
...
def objective(portfolio, days, setSize):
    ...
    """
    total = 1
    portsReturns = np.array(())
    marketReturns = np.array(())
    associatedRisk = 0
    for i in range(days.shape[1]) :
        total *= portfolio @ days[:,i]
        # associatedRisk += (np.cov(portfolio.flatten(), days[:,i].flatten())).sum()/np.var(days[:,i].flatten())
...
np.savetxt("./Data Sets/DRICORNK/{0}-Exchange-StartDate{1}-EndDate{2}.txt".format(exchange, startDate, ENDDate) ,wealth)
